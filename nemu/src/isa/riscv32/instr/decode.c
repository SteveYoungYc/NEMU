...
static def_DHelper(J) {
  sword_t simm = s->isa.instr.j.simm10_1 << 1 | s->isa.instr.j.simm11 << 11 |
                  s->isa.instr.j.simm19_12 << 12 | s->isa.instr.j.simm20 << 20;
...
static def_DHelper(B) {
  ...
  sword_t simm = (s->isa.instr.b.imm11 << 11) | (s->isa.instr.b.imm4_1 << 1) |
                  (s->isa.instr.b.simm10_5 << 5) | (s->isa.instr.b.simm12 << 12);
  decode_op_i(s, id_dest, simm, false);
}

def_THelper(base_cal) {
...
def_THelper(main) {
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 01100 11", R     , base_cal);
